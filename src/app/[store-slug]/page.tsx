...
export default async function StorefrontPage({ params }: { params: Promise<{ "store-slug": string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-10"><div className="mx-auto max-w-6xl"><header className="mb-10"><p className="text-sm font-semibold tracking-wide text-emerald-700">PAGOFÁCIL</p><h1 className="mt-2 text-4xl font-semibold">{store.store_name}</h1><p className="mt-2 text-slate-600">Compra fácil y paga con tus opciones preferidas.</p></header><div className="grid gap-8 lg:grid-cols-[1fr_320px]"><section className="grid gap-4 sm:grid-cols-2">{products.map((product) => <StorefrontProduct key={product.product_id} product={{ id: product.product_id, name: product.product_name, description: product.product_description, price_usd: product.price_usd }} />)}</section><StorefrontCart storeSlug={slug} paymentDetails={{ zelle_email: store.zelle_email, pago_movil_phone: store.pago_movil_phone, pago_movil_bank: store.pago_movil_bank, pago_movil_id: store.pago_movil_id, binance_pay_id: store.binance_pay_id }} /></div></div></main>;
}
