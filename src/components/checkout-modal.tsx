...
export function CheckoutModal({ storeSlug, paymentDetails }: { storeSlug: string; paymentDetails: PaymentDetails }) {
  ...
  async function submit() {
    ...
    if (!customerPhone.trim()) { setMessage("Indica tu número de WhatsApp para recibir la confirmación."); return; }
    ...
    body.append("store_slug", storeSlug); body.append("customer_name", customerName); body.append("customer_phone", customerPhone); body.append("payment_method", method); body.append("total_usd", String(total)); body.append("product_ids", JSON.stringify(items.map((item) => item.id))); body.append("proof", file);
